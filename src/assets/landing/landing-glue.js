(async () => {
  ...
  const ensureThemeLinkTag = () => {
    let tag = document.getElementById('theme-css-link');
    if (!tag) {
      tag = document.createElement('link');
      tag.id = 'theme-css-link';
      tag.rel = 'stylesheet';
      (document.head || document.documentElement).appendChild(tag);
    }
    return tag;
  ...
  const ThemeManager = {
    ...
    async switchTheme(name) {
      ...
      const entry = normalizeThemeEntry(this.cssData[name]);
      ...
      if (entry && entry.type === 'url') {
        ...
        styleTag.textContent = entry ? entry.value : '';
      }
