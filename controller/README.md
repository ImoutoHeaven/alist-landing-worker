...
- **配置**：示例位于 `config.yaml`（每个字段已标注用途、枚举/默认值/取值范围），顶层包含 `apiToken/bootstrapVersion/rulesVersion`，`envs.<env>` 下依次是 `common/landing/download/powdet/slotHandler`。staging/prod 结构一致，实际部署需按环境替换 token、上游地址等敏感字段。
...
- 依据 `config.yaml` 内联注释更新各字段；staging 样例已解释每个键的含义，prod 保持同结构并替换真实值。
